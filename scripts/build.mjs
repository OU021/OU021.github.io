...
const photography = `<aside class="photo-note"><div><a href="./photography/">${icon('camera')}Photography ${icon('arrow')}</a><p class="preview-hint">Moments I’ve photographed.</p></div><div class="photo-strip" role="group" aria-label="Selected photographs">${photoPreviews.map(p=>`<a class="photo-print" href="./${p.src}" data-figure data-album="home" ${photoAttributes(p)} aria-label="View photograph: ${esc(p.title||p.alt)}"><span class="photo-print-paper"><img src="./${p.src}" width="${p.width}" height="${p.height}" alt="${esc(p.alt)}" loading="lazy">${photoStamp(p)}</span><span class="preview-caption" aria-hidden="true">${esc(p.title||p.alt)}</span></a>`).join('')}</div></aside>`;
const previewBooks = d.bookshelfPreview.map(id=>books.find(book=>book.id===id)).filter(Boolean);
const bookPreview = `<aside class="photo-note book-note"><div><a href="./bookshelf/">${icon('book')}Bookshelf ${icon('arrow')}</a><p class="preview-hint">Books I’ve enjoyed.</p></div>${renderBookPreviews(previewBooks)}</aside>`;
writeFileSync(new URL('index.html',root),layout('Zhilin Ou — 3D Vision & Embodied AI','',hero+about+news()+papers+experience+education+awards+`<section class="personal-previews" aria-labelledby="beyond-research-title"><header class="personal-previews-heading"><h2 class="eyebrow" id="beyond-research-title">Beyond research</h2><p class="preview-hint">Click a photo, book, or exhibition to explore.</p></header>${photography}${bookPreview}<aside class="photo-note exhibition-note"><div><a href="./exhibitions/">${icon('gallery')}Exhibitions ${icon('arrow')}</a><p class="preview-hint">Exhibitions I’ve visited.</p></div><div class="exhibition-strip"><div class="exhibition-miniatures" role="group" aria-label="Selected exhibition photographs">${visits[0].photos.slice(0,3).map(p=>`<a class="exhibition-miniature" href="./exhibitions/${visits[0].id}/" aria-label="Explore ${esc(visits[0].venue)}: ${esc(p.alt)}"><img src="./${p.src}" width="${p.width}" height="${p.height}" alt="${esc(p.alt)}" loading="lazy"></a>`).join('')}</div></div></aside></section>`));

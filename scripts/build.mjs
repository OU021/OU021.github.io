...
<header class="site-header"><nav class="navigation shell" aria-label="Main navigation"><a class="wordmark" href="${home}"><img class="site-mark" src="${base}assets/kapok.svg" width="25" height="25" alt="" aria-hidden="true">Zhilin Ou</a><div class="nav-links">${navItems.map(n=>`<a href="${home}#${n.toLowerCase()}">${n}</a>`).join('')}<details><summary>More</summary><div class="dropdown"><a href="${home}photography/"${path==='photography/'?' aria-current="page"':''}>${icon('camera')}Photography</a></div></details></div></nav></header>
...
const hero = `<section class="hero" aria-label="Profile"><div class="hero-copy"><div class="identity"><div class="portrait-frame"><img class="portrait" src="./${esc(d.portrait.src)}" width="${d.portrait.width}" height="${d.portrait.height}" alt="${esc(d.portrait.alt)}" fetchpriority="high"></div><div><p class="greeting">${esc(d.greeting)}</p><h1>Zhilin Ou<span lang="zh">欧芝麟</span></h1></div></div><p class="role">${esc(d.role)}</p><p class="affiliation">${esc(d.affiliation)}</p><p class="interests">${d.interests.map(esc).join('<span aria-hidden="true"> · </span>')}</p><div class="hero-actions"><a class="button primary" href="#papers"><span class="action-icon">${icon('papers')}</span><span class="action-label">View papers</span></a>${Object.entries(d.links).filter(([,v])=>v).map(([k,v])=>`<a class="button" href="${esc(v)}"><span class="action-icon">${k==='GitHub'?githubMark:icon(k==='Email'?'mail':'cap')}</span><span class="action-label">${esc(k)}</span></a>`).join('')}</div></div><div class="hero-art" aria-describedby="scene-description"><div class="scene-view"><img class="scene-fallback" src="./assets/scene/camera.webp?v=${revision('scene/camera.webp')}" width="960" height="680" alt="An original camera-shaped point cloud with a detailed lens, viewfinder and camera body"><canvas id="point-field" data-src="${storyAssets.camera}" data-story="${esc(JSON.stringify(storyAssets))}" aria-hidden="true"></canvas><img class="scene-photograph" src="./assets/photos/digital7.webp" width="1500" height="1000" alt="${esc(d.photoStoryAlt)}" decoding="async" aria-hidden="true"></div><p class="sr-only" id="scene-description">A slow point-cloud loop connects a camera, Zhilin Ou’s photograph of a red kapok flower, a flower illustration, and Baymax gently examining the flower. The flower geometry is an illustration rather than a reconstruction of the photograph.</p><div class="art-footer"><p class="scene-story">${sceneStory}</p><button class="motion-toggle" type="button" aria-label="Pause point cloud motion" aria-pressed="false" hidden>${icon('pause')}</button></div></div></section>`;
const about = section('about','About',`<div class="about-copy">${d.about.map(p=>`<p>${withLabLink(p).replace('Prof. Wenjing Ma',link('https://marvinquiet.github.io/','Prof. Wenjing Ma')).replace('Prof. Zhihui Wang',link('https://scholar.google.com/citations?user=r-ZsDRQAAAAJ&hl=en','Prof. Zhihui Wang'))}</p>`).join('')}</div>`);
const papers = section('papers','Papers',d.papers.filter(p=>p.public && /^https:\/\//.test(p.url)).map(p=>`<article class="paper">${p.figure?`<a class="paper-figure" href="./${p.figure.src}" data-figure aria-label="Enlarge ${esc(p.title)} framework"><img src="./${p.figure.src}" width="${p.figure.width}" height="${p.figure.height}" alt="${esc(p.figure.alt)}" loading="lazy"><span class="figure-expand">${icon('expand')}<span>View framework</span></span></a>`:''}<div class="paper-copy"><p class="paper-source">${esc(p.source)} <span>·</span> ${p.year}</p><h3>${link(p.url,p.title)}</h3><p class="authors">${p.authors.map(a=>a===d.name?`<strong>${esc(a)}</strong>`:esc(a)).join(', ')}</p><a class="paper-link" href="${esc(p.url)}">${icon('papers')}Read paper ${icon('arrow')}</a></div></article>`).join(''));
...
const awards = section('awards','Awards',`<ul class="award-list">${d.awards.map(a=>`<li><time datetime="${a.year}">${a.year}</time><span>${esc(a.title)}</span></li>`).join('')}</ul>`);
function news(){return d.news.enabled && d.news.items.length?section('news','News',`<ul class="award-list">${d.news.items.map(n=>`<li><time>${esc(n.date)}</time><span>${n.url?link(n.url,n.text):esc(n.text)}</span></li>`).join('')}</ul>`):'';}
const photography = `<aside class="photo-note"><div><p class="eyebrow">Beyond research</p><a href="./photography/">${icon('camera')}Photography ${icon('arrow')}</a></div><a class="photo-strip" href="./photography/" aria-label="Explore photography">${['assets/photos/digital7.webp','assets/photos/digital1.webp'].map(src=>d.photographs.find(p=>p.src===src)).map(p=>`<img src="./${p.src}" width="${p.width}" height="${p.height}" alt="${esc(p.alt)}" loading="lazy">`).join('')}</a></aside>`;

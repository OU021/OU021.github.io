...
function layout(title,path,body) {
 ...
 const desc = isError?'This page wandered off. Return to Zhilin Ou’s academic homepage.':path==='bookshelf/'?'A few books Zhilin Ou loves, beyond research.':path==='photography/'?'Photography by Zhilin Ou.':'Zhilin Ou, MPhil student in Artificial Intelligence at CUHK-Shenzhen. Research in embodied AI, robot learning, and 3D vision.';
...
<footer class="footer shell${!path?' footer-home':''}">${!path?`<div class="footer-invitation"><p>Always happy to talk about robots, books, or photography.</p><a href="${esc(d.links.Email)}">Say hello ${icon('arrow')}</a></div><div class="footer-meta">`:''}<span>© 2026 Zhilin Ou</span><a href="${path?home:home+'photography/'}">${path?'Academic homepage':'Photography'} ${icon('arrow')}</a>${!path?'</div>':''}</footer>
</body></html>\n`;
}
...
const sceneStory = esc(d.sceneStory);
const sceneReference = d.sceneReference ? ` ${esc(d.sceneReference)}` : '';
const sceneCaption = d.sceneCaption ? `<p class="scene-caption">${esc(d.sceneCaption).replace(/\bphotography\b/, '<a class="scene-photography-link" href="./photography/">photography</a>')}</p>` : '';

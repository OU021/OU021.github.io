...
import {renderBookshelf} from './bookshelf.mjs';
import {renderPhotography, photoAttributes, photoLanguageToggle} from './photography.mjs';
...
function layout(title,path,body) {
 ...
 const home = path?'../':'./';
 const desc = path==='bookshelf/'?'A few books Zhilin Ou loves, beyond research.':path==='photography/'?'Photography by Zhilin Ou.':'Zhilin Ou, MPhil student in Artificial Intelligence at CUHK-Shenzhen. Research in embodied AI, robot learning, and 3D vision.';
 return `<!doctype html>
<html lang="en"><head>
...
${path==='bookshelf/'?`<link rel="stylesheet" href="${base}assets/bookshelf.css?v=${revision('bookshelf.css')}"><script src="${base}assets/bookshelf.js?v=${revision('bookshelf.js')}" defer></script>`:''}
...
const photography = `<aside class="photo-note"><div><p class="eyebrow">Beyond research</p><a href="./photography/">${icon('camera')}Photography ${icon('arrow')}</a><p class="preview-hint">Click a photo for its story.</p></div><div class="photo-strip" role="group" aria-label="Selected photographs">${photoPreviews.map(p=>`<a class="photo-print" href="./${p.src}" data-figure data-album="home" ${photoAttributes(p)} aria-label="View photograph: ${esc(p.title||p.alt)}"><img src="./${p.src}" width="${p.width}" height="${p.height}" alt="${esc(p.alt)}" loading="lazy"><span class="preview-caption" aria-hidden="true">${esc(p.title||p.alt)}</span></a>`).join('')}</div></aside>`;
const previewBooks = d.bookshelfPreview.map(id=>books.find(book=>book.id===id)).filter(Boolean);
const bookPreview = `<aside class="photo-note book-note"><div><p class="eyebrow">On my shelf</p><a href="./bookshelf/">${icon('book')}Bookshelf ${icon('arrow')}</a><p class="preview-hint">Click a cover to explore the book.</p></div><div class="book-strip" role="group" aria-label="Selected books">${previewBooks.map(book=>`<a class="book-pick" href="./bookshelf/#book-${esc(book.id)}" aria-label="Read about ${esc(book.en.title)} — ${esc(book.title)}"><img src="./${book.cover.src}?v=${book.cover.version}" width="${book.cover.width}" height="${book.cover.height}" alt="${esc(book.title)} — book cover" loading="lazy"><span class="preview-caption" lang="zh-Hant" aria-hidden="true">${esc(book.title)}</span></a>`).join('')}</div></aside>`;
writeFileSync(new URL('index.html',root),layout('Zhilin Ou — Embodied AI & Robot Learning','',hero+about+news()+papers+experience+education+awards+`<div class="personal-previews">${photography}${bookPreview}</div>`));

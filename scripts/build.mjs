...
const photography = `<aside class="photo-note"><div><p class="eyebrow">Beyond research</p><a href="./photography/">${icon('camera')}Photography ${icon('arrow')}</a></div><div class="photo-strip" role="group" aria-label="Selected photographs">${photoPreviews.map(p=>`<a class="photo-print" href="./${p.src}" data-figure data-album="home" ${photoAttributes(p)} aria-label="View photograph: ${esc(p.title||p.alt)}"><img src="./${p.src}" width="${p.width}" height="${p.height}" alt="${esc(p.alt)}" loading="lazy"><span class="preview-caption" aria-hidden="true">${esc(p.title||p.alt)}</span></a>`).join('')}</div></aside>`;
const previewBooks = ['bright-night','flatland','kitchen'].map(id=>books.find(book=>book.id===id)).filter(Boolean);
const bookPreview = `<aside class="photo-note book-note"><div><p class="eyebrow">On my shelf</p><a href="./bookshelf/">${icon('book')}Bookshelf ${icon('arrow')}</a></div><div class="book-strip" role="group" aria-label="Selected books">${previewBooks.map(book=>`<a class="book-pick" href="./bookshelf/#book-${esc(book.id)}" aria-label="Read about ${esc(book.en.title)} — ${esc(book.title)}"><img src="./${book.cover.src}?v=${book.cover.version}" width="${book.cover.width}" height="${book.cover.height}" alt="${esc(book.title)} — book cover" loading="lazy"><span class="preview-caption" lang="zh-Hant" aria-hidden="true">${esc(book.title)}</span></a>`).join('')}</div></aside>`;

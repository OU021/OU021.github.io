...
`researchVision` states the aspiration for embodied agents that continually learn, improve themselves, and support people with warmth and care. It appears once inside the profile panel between research interests and contact links. `sceneReference` introduces Zhilin as a photographer and explains how the animation brings together photography and this vision for embodied AI, with Baymax as a concrete image of warmth and care; its first mention of photography links to the gallery. `sceneCaption` identifies the personal photograph. These passages use no additional box or disclosure. About retains the academic background, with institutions and advisor links given a little more weight for scanning.
